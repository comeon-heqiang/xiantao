<template>
  <div>
    ...
    <div @click="modalShow=true">
      ...
      >
        <van-col span="23">
          <template v-if="address.streetName">
            <van-row
              type="flex"
              class="name"
            >
  ...
  methods: {
      ...
      axios({
        ...
        .then(res => {
          if (res.data.code == 200 && res.data.message) {
            ...
            this.address =
              this.addressList.find(item => item.isDefault) ||
              this.addressList[0] ||
              "";
          }
        })
    ...
    getGoodsList() {
      axios({
        ...
        .then(res => {
          if (res.data.code == 200 && res.data.message) {
            this.goodsList = res.data.message.filter(item => item.checked == 1);
          }
        })
        .catch(err => {
          console.log(err);
